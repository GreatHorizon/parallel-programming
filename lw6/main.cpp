...
struct ThreadParams
{
    ...
    ThreadParams(uint32_t startW, uint32_t endW, uint32_t startH,
        ...
        uint32_t number, int count, Bitmap* bmp, std::ostream* oStream, std::chrono::steady_clock::time_point start, LogBuffer* buffer)
        : startWidth(startW)
        , startHeight(startH)
        , endHeight(endH)
        , endWidth(endW)
...
void blur(int radius, ThreadParams params)
{
    ...
    for (int i = params.startHeight; i < params.endHeight; ++i)
    {
        for (int j = params.startWidth; j < params.endWidth; ++j)
        {
            ...
            std::string logStr = std::to_string(elapsed.count()) + " " + std::to_string(params.threadNumber) + "\n";
            *params.out << logStr;
            params.logBuffer->WriteLog(logStr);
...
void RunThreads(Args const& args, Bitmap& bmp, std::chrono::steady_clock::time_point& startTime, LogBuffer* buffer)
{
    ...
    for (unsigned i = 0; i < args.threadsCount; i++)
    {
        ...
        threadParams[i] = ThreadParams(startWidth, endWidth, startHeight, endHeight, squareWidth, squareHeight,
            extraWidth, extraHeight, i, args.threadsCount, &bmp, &out, startTime, buffer);
...
int main(int argc, const char* argv[])
{
    try
    {
        ...
        RunThreads(args, bmp, startTime, buffer);
        bmp.save(argv[2]);
        auto end = std::chrono::high_resolution_clock::now();
